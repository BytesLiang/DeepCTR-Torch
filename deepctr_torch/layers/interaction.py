...
class CrossLayer(nn.Module):
    ...
    def __init__(self, in_features, parameterization='vector', seed=1024, device='cpu'):
        super(CrossLayer, self).__init__()
        self.parameterization = parameterization
        if self.parameterization == 'vector':
            # weight in DCN.  (in_features, 1)
            self.kernels = nn.Parameter(torch.Tensor(in_features, 1))
        elif self.parameterization == 'matrix':
            # weight matrix in DCN-M.  (in_features, in_features)
            self.kernels = nn.Parameter(torch.Tensor(in_features, in_features))
        else:  # error
            raise ValueError("parameterization should be 'vector' or 'matrix'")
    ...
    def forward(self, x_0, x_l):
        ...
        x_l = x_l.unsqueeze(2)
        if self.parameterization == 'vector':
            xl_w = torch.tensordot(x_l, self.kernels, dims=([1], [0]))
            dot_ = torch.matmul(x_0, xl_w)
            x_l = dot_ + self.bias + x_l
        elif self.parameterization == 'matrix':
            xl_w = torch.matmul(self.kernels, x_l)  # W * xi  (bs, in_features, 1)
            dot_ = xl_w + self.bias  # W * xi + b
            x_l = x_0 * dot_ + x_l  # x0 · (W * xi + b) +xl  Hadamard-product
        else:  # error
            raise ValueError("parameterization should be 'vector' or 'matrix'")
        x_l = torch.squeeze(x_l, dim=2)
...
class AutomaticDiscretization(nn.Module):
    def __init__(self, H=20, alpha=0.1, tau=1, use_bn=False):
